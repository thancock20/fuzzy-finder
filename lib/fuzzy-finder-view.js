...
import path, { relative, basename, dirname } from 'path'
...
export default class FuzzyFinderView {
  constructor () {
    ...
    const getSelectedFilePathAndClose = (noIndex = false) => {
      let {filePath} = this.selectListView.getSelectedItem()
      this.cancel()

      if (noIndex && basename(filePath).match(/^index\./))
        filePath = dirname(filePath)

      return filePath
    }

    const getRelativePath = (sourcePath, destinationPath) => {
    ...
    const generateImport = () => {
      const generateImportCode = (relativePath, destinationPath, editor) => {
        relativePath = relativePath.replace(/\.jsx?$/, '')

        const selection = editor.getLastSelection()
        let identifierName = selection.getText()
        if (!identifierName) {
          identifierName = basename(destinationPath).replace(/\..+/, '')
          identifierName = _.capitalize(_.camelize(identifierName))

          return `import ${identifierName} from '${relativePath}';\n`
        }
      }

      const destinationPath = getSelectedFilePathAndClose(true)
      const editor = atom.workspace.getActiveTextEditor()
      const sourcePath = editor.getPath()
      const relativePath = getRelativePath(sourcePath, destinationPath)
      const code = generateImportCode(relativePath, destinationPath, editor)
      editor.insertText(code)
    }

    const generateMarkdownLink = () => {
      const generateLinkCode = (relativePath, destinationPath, editor) => {
        const selection = editor.getLastSelection()
        let linkText = selection.getText()
        if (!linkText) {
          linkText = basename(destinationPath).replace(/\..+/, '')
          linkText = _.uncamelcase(linkText)

          return `[${linkText}](${relativePath})`
        }
      }

      const destinationPath = getSelectedFilePathAndClose()
      const editor = atom.workspace.getActiveTextEditor()
      const sourcePath = editor.getPath()
      const relativePath = getRelativePath(sourcePath, destinationPath)
      const code = generateLinkCode(relativePath, destinationPath, editor)
      editor.insertText(code)
    }
    ...
    atom.commands.add(this.selectListView.element, {
      'fuzzy-finder:generate-import': generateImport,
      'fuzzy-finder:generate-markdown-link': generateMarkdownLink,
      'fuzzy-finder:paste-file-path': pasteFilePath,
